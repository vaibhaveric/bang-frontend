...
(function() {
  ...
  AdminPages.orders._setStatus = async function(id, status) {
    try {
      if (window.ADMIN_API) {
        await ADMIN_API.updateOrderStatus(id, status);
      } else {
        const orders = BB_APP.orders.read();
        const o = orders.find(x => x.id === id);
        if (o) { o.status = status; localStorage.setItem("bb_orders_v1", JSON.stringify(orders)); }
      }
